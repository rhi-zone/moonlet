//! Dynamic plugin loading for moonlet.
//!
...
/// Plugin metadata returned by moonlet_plugin_info().
#[repr(C)]
...
impl PluginLoader {
    ...
    pub fn new() -> Self {
        ...
        search_paths.push(PathBuf::from(".moonlet/plugins"));

        // User-local plugins
        if let Some(home) = dirs::home_dir() {
            search_paths.push(home.join(".moonlet/plugins"));
        }
        ...
        {
            // <exe_dir>/../lib/moonlet/plugins (standard FHS layout)
            search_paths.push(exe_dir.join("../lib/moonlet/plugins"));
            // <exe_dir>/plugins (flat layout)
            search_paths.push(exe_dir.join("plugins"));
        }

        // System plugins (Linux)
        search_paths.push(PathBuf::from("/usr/lib/moonlet/plugins"));
        search_paths.push(PathBuf::from("/usr/local/lib/moonlet/plugins"));
    ...
    /// and calls luaopen_moonlet_{name} to register it with Lua.
    pub fn load(&mut self, lua: &Lua, name: &str) -> LuaResult<()> {
        ...
        let open_fn_name = format!("luaopen_moonlet_{}", name);
    ...
    fn get_plugin_info(&self, library: &Library, name: &str) -> LuaResult<(String, String)> {
        let info_fn: Result<Symbol<unsafe extern "C" fn() -> PluginInfo>, _> =
            unsafe { library.get(b"moonlet_plugin_info") };
...
fn library_filename(name: &str) -> String {
    #[cfg(target_os = "linux")]
    {
        format!("libmoonlet_{}.so", name)
    }
    #[cfg(target_os = "macos")]
    {
        format!("libmoonlet_{}.dylib", name)
    }
    #[cfg(target_os = "windows")]
    {
        format!("moonlet_{}.dll", name)
    }
}
...
mod tests {
    ...
    fn test_library_filename() {
        ...
        #[cfg(target_os = "linux")]
        assert_eq!(name, "libmoonlet_fs.so");
        #[cfg(target_os = "macos")]
        assert_eq!(name, "libmoonlet_fs.dylib");
        #[cfg(target_os = "windows")]
        assert_eq!(name, "moonlet_fs.dll");
    }
