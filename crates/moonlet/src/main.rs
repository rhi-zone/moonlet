//! moonlet CLI - Run Lua scripts with plugin integrations.

use clap::{Parser, Subcommand};
use moonlet_lua::{RequireConfig, Runtime};
use schemars::JsonSchema;
use serde::Deserialize;
use std::path::{Path, PathBuf};

#[derive(Parser)]
#[command(name = "moonlet", about = "Lua runtime with plugin integrations")]
struct Cli {
    #[command(subcommand)]
    command: Commands,
}
...
enum Commands {
    ...
    /// Initialize a new moonlet project
    Init {
        /// Project directory (defaults to current directory)
        #[arg(default_value = ".")]
        path: PathBuf,
...
fn handle_schema_flag() -> bool {
    ...
    if args.get(1).map(|s| s.as_str()) == Some("--schema") {
        let response = serde_json::json!({
            "config_path": ".moonlet/config.toml",
            "format": "toml",
...
fn run(project_path: &Path, entry_override: Option<&Path>, args: &[String]) -> Result<(), String> {
    ...
    runtime.add_plugin_path(project_path.join(".moonlet/plugins"));
    ...
    // Set moonlet.args as a Lua table
    let args_table = lua
        .create_table()
...
require_plugins = true    # Allow require("moonlet.sessions"), require("moonlet.llm"), etc.
...
# [caps.libsql]
# data = { path = "${PROJECT_ROOT}/.moonlet/data" }
...
"#;
    ...
    println!("  .moonlet/config.toml - configuration");
